...
var urlNotificationHtml = "notification.html";
...
chrome.runtime.onInstalled.addListener(function() {
    ...
    function defaultSettings(){
        ...
        var arraySettingsToStore = {
            "Domyslna": [timeWork,timeBreak,timeBreakLong,timeWorkQuantity],
            "Zestaw1": [0,0,0,0],
            "Zestaw2": [0,0,0,0],
            "Zestaw3": [0,0,0,0],
        ...
        console.log("zapisanie do domyslnych parametrów do localStorage", arraySettingsToStore);
...
//remove hostname from blacklist
chrome.runtime.onMessage.addListener(
    function(request, sender, sendResponse) {
	...
	function(request, sender, sendResponse) {
		...
		if(isStartPressed === false) {
			if (request.action === 'start_session') {
				...
				if(areParametersValid() === true) {
					console.log("Start-Flaga 1 - parameters are valid");
				...
				console.log("start_session - wrong_message");
			}
		}
		...
		function session(timeWork, timeBreak, timeBreakLong, timeWorkQuantity) {
			...
			var arraySession = [];  //array in which following countdown arguments are stored
									//loop to save following setting to a sessionArray
